...
Known Bugs/Disclaimers:
	- only super user (admins) can create location (for appointments)
	- nurses, doctors, and other admins are created by another admin
	- super user is required to get the server up and running. After the first admin is created, it can run independently.
	- calendar has not been set up for a patient to view a doctor's available time slots
	- messaging has not been implemented yet
	- nurses are able to view the button for add diagnosis under patient list
	- canceled appointments are still apearing in the calendar.
...
Basic(patient) execution and usage instructions (logins & passwords):
	To run
...
Information is the same for Doctors and Nurses other than account inital creation, which is done by an admin.
